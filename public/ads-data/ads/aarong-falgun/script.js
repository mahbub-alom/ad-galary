...
document.querySelector('.ppm_closeBtn_takeover').addEventListener('click', (e) => {
    ...
    gsap.set('.ppm_closeBtn_takeover', {
        display: 'none'
    })
}
)
...
function beginningAnimation() {
    gsap.set('.ppm_container', {
        display: 'block'
    })
    gsap.fromTo('.ppm_video_shape', {
        display: 'none',
        opacity: 0,
        scale: .5
    }, {
    ...
    gsap.timeline().fromTo('.ppm_text_1', {
        display: 'none',
        opacity: 0,
        scale: .8,
        y: '300px'
...
function animateFloatingElements(groupNumber) {
    gsap.to(`.ppm_flying${groupNumber}_elms`, {
        display: 'block'
    })
...
document.querySelector('.ppm_cta').addEventListener('click', (e) => {
    window.open('https://www.aarong.com/newspost/aarong-falgun-24.html')
}
)
    ...
    on: {
        slideChange: function() {
            ...
            this.animationTimeout = setTimeout( () => {
                ...
                animateFloatingElements(activeIndex);
            }
            , 50);
        },
        init: function() {
            // Trigger animation for first slide on init
            animateFloatingElements(1);
        }
    }
});
