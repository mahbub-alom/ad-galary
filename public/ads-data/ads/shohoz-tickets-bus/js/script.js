...
document.querySelector(".ppm_cta").addEventListener("click", () => {
  ...
  let dateFrom = new Date(dateSelect.value);
  const months = [
    "Jan",
    "Feb",
    "Mar",
  ...
  dateFrom = `${dateFrom.getDate().toString().padStart(2, "0")}-${
  ...
  if (cityFrom && cityTo && dateFrom) {
    ...
    window.open(
      `https://www.shohoz.com/bus-tickets/booking/bus/search?fromcity=${cityFrom}&tocity=${cityTo}&doj=${dateFrom}&dor=`
    );
  }
});
...
document.getElementById("callToBookImage").addEventListener("click", () => {
  ...
  if (isMobile) {
    ...
    window.open(
      "https://bdtickets.com/?utm_source=pp&utm_campaign=pp+bdtickets&utm_content=pp+banner",
      "_blank"
    );
  }
});
